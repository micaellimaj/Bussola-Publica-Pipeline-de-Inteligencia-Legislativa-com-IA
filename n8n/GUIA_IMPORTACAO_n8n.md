...
3. Configure as **credenciais** (os campos vêm com placeholders):
   - **Postgres / Supabase:** crie a credencial *Postgres* usando o **Session Pooler** (no painel do Supabase: botão **Connect** → aba *Session pooler*). Host `aws-1-sa-east-1.pooler.supabase.com` (confirme no painel; pode ser `aws-0-...`), database `postgres`, usuário `postgres.thtdslaojkzbnzvbfjvu` (ref do projeto após o ponto), porta `5432`, SSL ligado.
     ⚠️ **Não use o host direto** `db.<ref>.supabase.co`: ele só resolve em IPv6 e o container Docker do n8n não tem IPv6 — dá o erro "Host não encontrado".
